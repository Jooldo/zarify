...
-- Fix the get_next_manufacturing_order_number function to remove FOR UPDATE with aggregates
...
BEGIN
    ...
    LOOP
        ...
        BEGIN
            -- Get the highest order number for this merchant (without FOR UPDATE)
            SELECT COALESCE(
                MAX(
                    CASE 
