...
-- Update the get_next_manufacturing_order_number function to handle rework orders properly
...
BEGIN
    ...
    LOOP
        ...
        BEGIN
            -- Get the highest order number for this merchant, including rework orders
            SELECT COALESCE(
                MAX(
                    CASE 
                        WHEN order_number ~ '^MO[0-9]+(-R[0-9]+)?$' 
                        THEN 
                            CASE 
                                WHEN order_number ~ '^MO[0-9]+-R[0-9]+$' THEN
                                    -- For rework orders, extract the base number
                                    CAST(SUBSTRING(order_number FROM 3 FOR POSITION('-' IN order_number) - 3) AS INTEGER)
                                ELSE
                                    -- For regular orders, extract the full number
                                    CAST(SUBSTRING(order_number FROM 3) AS INTEGER)
                            END
                        ELSE 0
                    END
                ), 0
...
-- Also, let's create a function specifically for generating rework order numbers
...
BEGIN
    ...
    LOOP
        ...
        BEGIN
            ...
            FROM manufacturing_orders 
            WHERE merchant_id = current_merchant_id
            AND order_number LIKE base_order_number || '-R%';
            
            -- Create the rework order number
