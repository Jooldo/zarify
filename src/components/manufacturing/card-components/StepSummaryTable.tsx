    ...
    <div className="bg-white/70 backdrop-blur-sm rounded-lg p-3 border border-gray-200/50">
      ...
      <div className="space-y-2">
        {stepSummaries.map((summary) => (
          <div key={summary.stepName} className="grid grid-cols-6 gap-2 text-xs">
            ...
            <div className="text-center bg-emerald-50 px-1 py-1 rounded text-emerald-700 text-xs font-semibold">
              {summary.weightReceived > 0 ? `${summary.weightReceived.toFixed(1)}kg` : '—'}
            </div>
            <div className="text-center bg-orange-50 px-1 py-1 rounded text-orange-700 text-xs font-semibold">
              {summary.reworkAssigned > 0 ? `${summary.reworkAssigned.toFixed(1)}kg` : '—'}
            </div>
            <div className="text-center bg-red-50 px-1 py-1 rounded text-red-700 text-xs font-semibold">
              {summary.reworkReceived > 0 ? `${summary.reworkReceived.toFixed(1)}kg` : '—'}
            </div>
            <div className="text-center bg-purple-50 px-1 py-1 rounded text-purple-700 text-xs font-semibold">
              {summary.reworkRequested > 0 ? `${summary.reworkRequested.toFixed(1)}kg` : '—'}
            </div>
          </div>
        ))}
      </div>
      <div className="grid grid-cols-6 gap-2 mt-2 pt-2 border-t border-gray-200 text-xs font-medium text-gray-500">
        <div>Step</div>
        ...
        <div className="text-center">RW Req</div>
      </div>
    </div>
  );
};
