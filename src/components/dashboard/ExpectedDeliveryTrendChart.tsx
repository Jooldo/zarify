...
import { format, subDays, subWeeks, subMonths, startOfDay, startOfWeek, startOfMonth, addDays } from 'date-fns';
...
interface ExpectedDeliveryTrendChartProps {
  orders: any[];
  loading: boolean;
}
...
const ExpectedDeliveryTrendChart = ({ orders, loading }: ExpectedDeliveryTrendChartProps) => {
  ...
  const { chartData, categories } = useMemo(() => {
    ...
    if (dateRange === 'custom' && customDateFrom && customDateTo) {
      ...
      switch (dateRange) {
        ...
        case 'thisWeek':
          switch (timeInterval) {
            case 'daily':
              periods = Array.from({ length: 7 }, (_, i) => addDays(now, i));
              formatString = 'MMM dd';
              groupByFunction = (date: Date) => format(startOfDay(date), 'yyyy-MM-dd');
              break;
            case 'weekly':
              periods = Array.from({ length: 4 }, (_, i) => addDays(now, i * 7));
              formatString = 'MMM dd';
              groupByFunction = (date: Date) => format(startOfWeek(date, { weekStartsOn: 1 }), 'yyyy-MM-dd');
              break;
            default:
        ...
        case 'thisMonth':
          switch (timeInterval) {
            case 'daily':
              periods = Array.from({ length: 30 }, (_, i) => addDays(now, i));
              formatString = 'MMM dd';
              groupByFunction = (date: Date) => format(startOfDay(date), 'yyyy-MM-dd');
              break;
            case 'weekly':
              ...
              break;
            case 'monthly':
              periods = Array.from({ length: 6 }, (_, i) => addDays(now, i * 30));
              formatString = 'MMM yyyy';
              groupByFunction = (date: Date) => format(startOfMonth(date), 'yyyy-MM');
              break;
          }
    ...
    // Filter orders that have expected_delivery dates
    const ordersWithDeliveryDates = orders.filter(order => order.expected_delivery);

    // Filter orders by category and product code if selected
    const filteredOrders = ordersWithDeliveryDates.filter(order => {
      // Category filter
    ...
    // Aggregate order data by expected delivery period
    filteredOrders.forEach(order => {
      const deliveryDate = new Date(order.expected_delivery);
      const periodKey = groupByFunction(deliveryDate);
  ...
  if (!ordersWithDeliveryDates.length) {
    return (
      <Card className="w-full">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
        ...
        <CardContent>
          <div className="h-96 flex items-center justify-center">
            <div className="text-center text-muted-foreground">
              ...
              <p>No orders with expected delivery dates</p>
              <p className="text-sm">Expected delivery dates will appear here once set</p>
            </div>
          </div>
        </CardContent>
      </Card>
...
export default ExpectedDeliveryTrendChart;
